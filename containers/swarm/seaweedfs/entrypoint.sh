...
case "$SERVICE" in
    'master')
        ...
        if [ ! -z "$(echo $MAX_VOLUME_SIZE)" ]; then ARGS="$ARGS -volumeSizeLimitMB=$MAX_VOLUME_SIZE"; fi
        if [ ! -z "$(echo $REPLICATION)" ]; then ARGS="$ARGS -defaultReplication=$REPLICATION"; fi
    ...
    'volume')
        ARGS="$ARGS -port=80"
        if [ ! -z "$(echo $DATACENTER)" ]; then ARGS="$ARGS -dataCenter=$DATACENTER"; fi
        if [ ! -z "$(echo $RACK)" ]; then ARGS="$ARGS -rack=$RACK"; fi
        if [ ! -z "$(echo $MAX_VOLUMES)" ]; then ARGS="$ARGS -max=$MAX_VOLUMES"; fi
    ...
    's3')
        ...
        if [ ! -z "$(echo $DOMAIN_NAME)" ]; then ARGS="$ARGS --domainName=$DOMAIN_NAME"; fi
        while [ $(get_local_filer | wc -w) -lt 1 ]; do sleep 1; done
        ARGS="$ARGS -filer=$(get_local_filer)"
        ;;
    'webdav')
